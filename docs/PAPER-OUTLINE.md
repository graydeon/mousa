...
7. **Mode ablations** — verified vs enforced vs traced. Verified==enforced measured
   (phase 13); traced ranking-neutrality and the pack byte-budget coverage curve measured
   by the harness (`beir -report coverage`, phase 15; per-run request namespaces resolved
   the earlier blocker in phase 14). *Grounded: RESEARCH.md traced-curve section.*
