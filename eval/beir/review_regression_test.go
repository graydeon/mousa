...
// These tests convert the 2026-09-15 adversarial review probes from defect
// demonstrations into correct-behavior regression requirements.

// TestReviewChangedCorpusReuseRefused: reusing an indexed store whose corpus
...
func TestReviewChangedCorpusReuseRefused(t *testing.T) {
...
// TestReviewUnchangedCorpusReuseAccepted: the correspondence check must accept
// the exact content that indexed the store.
func TestReviewUnchangedCorpusReuseAccepted(t *testing.T) {
	dir, ingested := ingestFixture(t,
		map[string]string{"d1": "cat sat", "d2": "dog bark"},
		map[string]string{"q": "cat"},
		map[string]map[string]int{"q": {"d1": 1}})
...
// TestReviewJournalAccountingMatchesResult: the journal callback must observe
// the same accepted/considered accounting the returned result carries; the
// review reproduced a journaled 0/0 against a returned 1/1.
func TestReviewJournalAccountingMatchesResult(t *testing.T) {
	dir, ingested := ingestFixture(t,
		map[string]string{"d1": "cat sat", "d2": "dog bark"},
		map[string]string{"q": "cat"},
		map[string]map[string]int{"q": {"d1": 1}})
...
// full-path latency must be at least the search component (review finding R6).
