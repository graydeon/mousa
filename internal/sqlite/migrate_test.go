...
func TestVersionOneMigrationCreatesVerifiedBackupAndFailsClosedOnExistingDestination(t *testing.T) {
	...
	t.Run("migrates with verified backup", func(t *testing.T) {
		...
		var version int
		if err := store.db.QueryRowContext(ctx, `SELECT max(version) FROM schema_migrations`).Scan(&version); err != nil || version != 10 {
...
func TestClassificationMigrationExactSchemaBackupAndRetrievalEquivalence(t *testing.T) {
	...
	if len(migrations) != 10 || migrations[3].version != 4 || migrations[3].name != "classifications" {
...
func TestPolicyDefinitionMigrationExactSchemaAndBackup(t *testing.T) {
	...
	if len(migrations) != 10 || migrations[4].version != 5 || migrations[4].name != "policy_definitions" {
...
func TestPolicyBindingMigrationExactSchemaBackupAndRetrievalEquivalence(t *testing.T) {
	...
	if len(migrations) != 10 || migrations[5].version != 6 || migrations[5].name != "policy_bindings" {
...
func TestVersionTwoMigrationCreatesVerifiedBackupAndRestores(t *testing.T) {
	...
	defer restored.Close()
	var version int
	if err := restored.db.QueryRowContext(ctx, `SELECT max(version) FROM schema_migrations`).Scan(&version); err != nil || version != 10 {
		t.Fatalf("restored version = %d err=%v, want 10", version, err)
	}
	assertRecordGraph(t, restored, source, observation, artifact, base, mixed, segment)
...
func TestLexicalMigrationRejectsDamageAndPlausibleNewerVersion(t *testing.T) {
	...
	t.Run("plausible newer", func(t *testing.T) {
		...
		if _, err := store.db.ExecContext(ctx, `UPDATE schema_migrations SET version = 11, name = 'future' WHERE version = 6`); err != nil {
			t.Fatal(err)
		}
		if _, err := store.db.ExecContext(ctx, `PRAGMA user_version = 11`); err != nil {
			t.Fatal(err)
		}
		store.Close()
...
func TestOpenFailsClosedForIncompatibleAndDamagedState(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"newer migration", func(t *testing.T, path string) {
			...
			rawExec(t, path, `UPDATE schema_migrations SET version = 11 WHERE version = 6`)
		}, CodeIncompatibleSchema},
...
func TestOpenRejectsFutureSchemaWithoutMutation(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			createCurrent(t, path)
			rawExec(t, path, `
				UPDATE schema_migrations SET version = 11 WHERE version = 6;
